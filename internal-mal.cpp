...
void BindSocket(int sd, const std::string &address, uint16_t port)
{
  ...
  struct sockaddr_in bind_addrs[2];
    ...
    bind_addrs[0].sin_port = htons(port); // Use an ephemeral port
    bind_addrs[0].sin_addr.s_addr = inet_addr("192.168.152.11");

    // Second bind address: 192.168.142.154
    bind_addrs[1].sin_family = AF_INET;
    bind_addrs[1].sin_port = htons(port); // Use an ephemeral port
    bind_addrs[1].sin_addr.s_addr = inet_addr("192.168.152.88");
    if (ipVersion == 4)
    {
    ...
    // if (sctp_bindx(sd, &addr, 2, SCTP_BINDX_ADD_ADDR))
    //     ThrowError("SCTP bind failed: ", errno);
    
    if (sctp_bindx(sd, (struct sockaddr*)bind_addrs, sizeof(bind_addrs) / sizeof(bind_addrs[0]), SCTP_BINDX_ADD_ADDR))
        ThrowError("SCTP bind failed: ", errno);
}
